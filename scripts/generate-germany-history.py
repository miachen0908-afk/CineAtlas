...
SOURCE = Path("/Users/mia0908/2-教学工作/电影学教研室/2026-2027 上/科研项目/德国电影史.docx")
...
def event_from_block(title: str, paragraphs: list[str], stage_index: int, event_index: int) -> dict:
    ...
    event_id = f"de-s{stage_index + 1}-{start}-{event_index + 1}"
...
def main() -> None:
    ...
    for stage_index, start in enumerate(main_starts):
        ...
        stage = {
            "id": f"germany-stage-{stage_index + 1}",
            "title": title,
        ...
        if substage_starts:
            ...
            for sub_index, sub_start in enumerate(substage_starts):
                ...
                substages.append({
                    "id": f"germany-stage-{stage_index + 1}-substage-{sub_index + 1}",
    ...
    expected = {"stages": 8, "subStages": 19, "events": 172, "films": 37}
    if counts != expected:
    ...
    for stage_index, stage in enumerate(stages, start=1):
        ...
        (OUTPUT_DIR / f"de-stage-{stage_index}.ts").write_text(
            'import type { CinemaHistoryStage } from "@/types/cinema";\n\n'
            f"export const germanyStage{stage_index}: CinemaHistoryStage = {payload};\n",
            encoding="utf-8",
        )
    imports = "\n".join(
        f'import {{ germanyStage{stage_index} }} from "./de-stage-{stage_index}";'
        for stage_index in range(1, len(stages) + 1)
    )
    stage_names = ",\n    ".join(f"germanyStage{stage_index}" for stage_index in range(1, len(stages) + 1))
    (OUTPUT_DIR / "de.ts").write_text(
