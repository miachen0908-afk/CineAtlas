...
SOURCE = Path("/Users/mia0908/2-教学工作/电影学教研室/2026-2027 上/科研项目/俄罗斯电影史.docx")
...
def films_from_block(paragraphs: list[str], event_id: str) -> list[dict]:
    ...
    positions = [
        ...
        if (re.fullmatch(r"《.+》", text) or (text.startswith("代表影片：") and "《" in text))
        and pos + 1 < len(paragraphs)
        and paragraphs[pos + 1].startswith("原文片名：")
    ]
    for index, pos in enumerate(positions):
        text = paragraphs[pos]
        title_match = re.search(r"《(.+)》(?P<suffix>.*)", text)
        if not title_match:
            continue
        title = f"{title_match.group(1)}{title_match.group('suffix')}".strip()
        metadata = {}
...
def event_from_block(title: str, paragraphs: list[str], stage_index: int, event_index: int) -> dict:
    match = re.match(r"(?P<date>\d{4}(?:—(?:\d{4}|至今))?)(?:年(?:\d{1,2}月)?)?(?:｜|：)(?P<title>.+)", title)
    if not match:
        raise ValueError(f"Invalid event heading: {title}")
    start, end, _ = period(match.group("date"))
    event_title = match.group("title")
    event_id = f"ru-s{stage_index + 1}-{start}-{event_index + 1}"
    films = films_from_block(paragraphs, event_id)
    description_parts = []
    for text in paragraphs:
        if text == "代表电影" or text.startswith("资料来源") or re.fullmatch(r"《.+》", text) or text.startswith(("代表影片：《", "原文片名：", "内容简介：", "价值意义：")):
            break
...
def main() -> None:
    ...
    for stage_index, start in enumerate(main_starts):
        ...
        substage_starts = [
            ...
            if paragraphs[i].style.name.startswith("Heading 1")
        ...
        stage = {
            "id": f"russia-stage-{stage_index + 1}",
            "title": title,
        ...
        if substage_starts:
            ...
            for sub_index, sub_start in enumerate(substage_starts):
                ...
                substages.append({
                    "id": f"russia-stage-{stage_index + 1}-substage-{sub_index + 1}",
    ...
    expected = {"stages": 10, "subStages": 12, "events": 222, "films": 47}
    if counts != expected:
    ...
    for stage_index, stage in enumerate(stages, start=1):
        ...
        (OUTPUT_DIR / f"ru-stage-{stage_index}.ts").write_text(
            'import type { CinemaHistoryStage } from "@/types/cinema";\n\n'
            f"export const russiaStage{stage_index}: CinemaHistoryStage = {payload};\n",
            encoding="utf-8",
        )
    imports = "\n".join(
        f'import {{ russiaStage{stage_index} }} from "./ru-stage-{stage_index}";'
        for stage_index in range(1, len(stages) + 1)
    )
    stage_names = ",\n    ".join(f"russiaStage{stage_index}" for stage_index in range(1, len(stages) + 1))
    (OUTPUT_DIR / "ru.ts").write_text(
